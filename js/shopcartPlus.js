...
$(function(){
    ...
    var $tbodyInputs = $('tbody input[type=checkbox]'); //表格中的每一行全选框
    var $totalPriceInput = $('.totalPrice input[type=checkbox]'); // 计算总价中的全选框
    /*全选*/
    /* 
        1.给表格中的全选按钮 绑定点击事件 点击的时候 获取到它的选中状态（true / false)
   ...
   $('.add').click(function(){  //给增加绑定事件
       ...
       count++; //自增
       $(this).next().val(count); //把自增的的值 赋值给后面的输入框
    ...
    */
    function subTotal(dom,count){
       ...
       dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2))//把小计的效果渲染到对应的位置 保留2位小数
    }
    /*
      总计功能实现：（头部全选 计算总价全选  表格选择框+ - 删除 六个地方调用总计）
